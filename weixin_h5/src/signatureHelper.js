 ...
 * @LastEditTime: 2024-03-27 15:04:18
 */
...
export function buildPostAuthorizationHeader(appId, curCusId, dshoIdWx) {
    ...
    // 没有 dshoIdWx 就写死110
    const shoId = dshoIdWx ? dshoIdWx : '110'
...
export function buildGetAuthorizationHeader1(appId, curCusId) {
    const baseUri = '/v1/customer/getCustomerInfo';
    // const key = 'ycsc7ty8ioweyzpmy3rivcqdwo68gc9m'; // 测试
    const key = 'vpelct4wh9p6rgmy6c6bankxblag2rdd'; // 正式
    const nonce = Math.random().toString(36).substring(2, 12)
    const timestamp = Math.floor(Date.now() / 1000).toString()
...
// https://tm.sanfu.com/ms-sanfu-spi-customer/v1/customer/getCustomerInfo